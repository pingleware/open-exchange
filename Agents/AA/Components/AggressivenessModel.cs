...
namespace OPEX.Agents.AA.Components
{
    class AggressivenessModel
    {
        ...
        private readonly ComputeTauDelegate InnerComputeTau;
        ...
        public AggressivenessModel(AAAgent agent, OrderSide side, double limitPrice, double PMax)
        {
        ...
        public void UpdateLimitPrice(double p)
        {
            _logger.Trace(LogLevel.Debug, "UpdateLimitPrice. Limit price updated to {0}", p);
            _limitPrice = p;
        }
        ...
        {
            return (Side == OrderSide.Buy && (_limitPrice > estimatedPrice)) || (Side == OrderSide.Sell && (_limitPrice < estimatedPrice));
        }
