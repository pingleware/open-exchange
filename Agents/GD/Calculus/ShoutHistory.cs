    ...
    public class ShoutHistory
    {
        ...
        public double FindExp(double price, ShoutFilter shoutFilter, OrderSide orderSide, bool greaterThan)
        {
            ...
            foreach (PriceBucket pb in _priceBuckets.Values)
            {
                ...
                {
                    if (lastTimeStamp < shout.LocalTimeStamp)
                    {
                        lastTimeStamp = shout.LocalTimeStamp;
                    }

                    bool isAccepted = (shout.Accepted &&
                    (DateTime.Now.Subtract(shout.LocalTimeStamp).CompareTo(TimeSpan.FromSeconds(GracePeriodSecs)) <= 0));
                ...
                {
                    weightedSum += Math.Exp(-lastTimeStamp.Subtract(s.LocalTimeStamp).TotalSeconds / TAU);
                }
            }       
