    ...
    public class PriceBucketSide : PriceContainer, IEnumerator, IEnumerable
    {
        ...
        public string Dump()
        {
            ...
            {               
                ...
                sb.AppendFormat("({0} {1} {2} {3}) ", s.LocalTimeStamp.ToString("HH:mm:ss.ffffff"), accepted, buysell, s.Price);          
            }
        ...
        private bool HasGracePeriodExpired(Shout s)
        {
            ...
            DateTime timestamp = s.LocalTimeStamp;
            TimeSpan diff = now.Subtract(timestamp);
            int compare = diff.CompareTo(GracePeriod);
            return compare > 0;
        }
