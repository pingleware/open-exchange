...
namespace OPEX.DES.AA
{
    class AdaptiveComponent
    {
        ...
        public AdaptiveComponent(DESAAAgent agent, AggressivenessModel aggressivenessModel)
        {
        ...
        public AdaptiveComponent(DESAAAgent agent, OrderSide side, double limitPrice, AggressivenessModel aggressivenessModel)
        {
            _logger = new Logger(string.Format("AdaptiveComponent({0})", agent.Name));
        ...
        public OrderSide Side { set { _side = value; } }
        ...
        public bool UpdateShortTermFromInactivity(double estimatedPrice)
        {
            ...
            AggregatedDepthSnapshot snapshot = _agent.LastMarketData.LastSnapshot;
            if (snapshot == null)
            {
        ...
        public void UpdateShortTerm(Shout shout, double estimatedPrice)
        {
            ...
            else if (_side == OrderSide.Buy && shout.Side == OrderSide.Buy)
            {
                double bid = shout.Price;
            ...
            else if (_side == OrderSide.Sell && shout.Side == OrderSide.Sell)
            {
                double ask = shout.Price;
